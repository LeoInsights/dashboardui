            ...
            api.post('report', apiData, function(result) {
                ...
                if (!result.error) {
                    for (var columnId in result.columns) {
                        result.columns[columnId].formatter = format.get(
                            result.columns[columnId]
                        );
                    }
                    for (var i = 0; i < (result.mapping || {}).length; i++) {
                        result.mapping[i].formatter = format.get(
                            result.mapping[i]
                        );
                    }
                }
    ...
    this.downloadData = function(title, data, dataSource) {
        if (!data && dataSource) {
			...
			*/
            ...
            dataSource.rows.forEach(row => {
                data.push(
                    row.map((value, index) => {
                        return dataSource.mapping[index].formatter
                            ? dataSource.mapping[index].formatter(value)
                            : value;
                    })
                );
            });
        ...
        if (data) {
            ...
            if (!downloadForm.length) {
                downloadForm = $(
                    '<div id="downloadform" style="display: none"><a>download</a>'
                ).appendTo('body');
            }
            var d = data
                .map(function(r) {
