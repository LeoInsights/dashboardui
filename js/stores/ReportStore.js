	...
	getSort: function() {
		...
		if (_sortVars.length) {
			...
			var sorts = _sortVars.concat(_rowDims.map(function(id, index) {
				return {column: index, direction: 'asc', auto: true};
			}).filter(function(c) {
...
function addDimension(to, position, dimension, isUpdate) {
	...
	if (to == "column") {
		var arr = _columnDims;
		_columnHeaders.push({id: dimension, width: 150});
	} else {
		var arr = _rowDims;
		_rowHeaders.push({id: dimension, height: 38});
	}
...
function addMetric(to, position, metric, isUpdate) {
	...
	if (to == "column") {
		if (position === null) {
			_columnHeaders.push({id: metric, height: 38, type: "metric"});
		} else {
			_columnHeaders.splice(offset+position, 0, {id: metric, height: 38, type: "metric"});
		}
	} else {
		if (position === null) {
			_rowHeaders.push({id: metric, width: 120, type: "metric"});
		} else {
			_rowHeaders.splice(offset+position, 0, {id: metric, width: 120, type: "metric"});
		}
	}
	
	//auto filters
	var fact = metric.split(/[|.]/)[0];
...
function removeOtherMetrics(index) {
	var rowMetrics = _rowMetrics.splice(index, 1);
  ...
  var sendingArray;
	...
	if (type == "dimension") {
		...
		if (from == "column") {
			...
			_columnHeaders.push({type: "metrics", height: 38});

			//Send all the headers over and remove from it's origin
			for(var i = 0; i < _columnHeaders.length; i++) {
				if (_columnHeaders[i].type == "metric") {
					_rowHeaders.push({id: _columnHeaders[i].id, width: 120, type: "metric"});
			...
			_rowHeaders.push({type: "metrics", width: 120});

			//Send all the headers over and remove from it's origin
			for(var i = 0; i < _rowHeaders.length; i++) {
				if (_rowHeaders[i].type == "metric") {
					_columnHeaders.push({id: _rowHeaders[i].id, height: 38, type: "metric"});
