...
module.exports = function (element, chart, options, my) {
                    ...
                    events: {
                        ...
                        click: function (e) {
                            ...
                            if (events && events['leo-chart-click']) {
                                element.trigger('leo-chart-click', [
                                    e.chartX,
                                    e.chartY,
                                    this.plotBox,
                                    this.spacing,
                            ...
                            events: {
                                click: function (e) {
                                    ...
                                    if (events && events['leo-click']) {
                                        ...
                                        events &&
                                        events['leo-chart-click']
                                    ) {
                                        element.trigger('leo-chart-click', [
                                            e.chartX,
                                            e.chartY,
                                            this.series.chart.plotBox,
                                            this.series.chart.spacing,
                                            this.series.chart.margin,
                                        ]);
                                    } else {
                                        ...
                                        if (details.tableView) {
                                            ...
                                            headingHTML +=
                                                '<a class="leo-tableView icon-table" title="table view" data-params="' +
                                                params +
                                                '" data-user_clicked="' +
                                                encodeURIComponent(
                                                    JSON.stringify(leo)
                                        ...
                                        if (details.addFilter) {
                                            ...
                                            var dataValue = parseDetailsString(
                                                    details.addFilter.data
                                                        .value,
                                                    leo
                                                ),
                                                checkboxes = {};
                                            for (
                                                var i = 0;
                                                i <
                                                metricData.headers[0].length;
                                                i++
                                            ) {
                                                var value =
                                                    metricData.headers[0][i]
                                                        .value;
                                                checkboxes[value] =
                                                    value == dataValue;
                                            }
                                            ...
                                            headingHTML +=
                                                '<a class="leo-addFilter icon-filter" title="filter by" data-id="' +
                                                details.addFilter.data.id +
                                                '" data-value="' +
                                                details.addFilter.data.value +
                                                '" data-checkboxes="' +
                                                encodeURIComponent(
                                        ...
                                        chartDetails.htmlExpand(null, {
                                            ...
                                            headingText: parseDetailsString(
                                                headingHTML,
                                                leo
                                            ),
                                            maincontentText: parseDetailsString(
                                                details.text || '',
                                                leo
                                            ),
    ...
    that.build = function () {
        ...
        my.chart.showLoading("<i class='fa fa-spinner fa-spin fa-2x'></i>");
    };
    ...
    that.showLoading = function () {
        my.chart.hideNoData(); // this prevents bug where loading and No Data messages were overlapping.
        my.chart.showLoading("<i class='fa fa-spinner fa-spin fa-2x'></i>");
    };
    ...
    my.redraw = function () {
        if (my.dataSources.length > 0) {
            ...
            Object.keys(my.dataSources[0].columns || {}).forEach((columnId) => {
                ...
                if (column.color && typeof column.color == 'object') {
                    window.leo.charts.colors = $.extend(
            ...
            if (window.leo.defaultColors) {
                OptionActions.setDefaultColors(
                    'default',
                    window.leo.defaultColors
                );
            }

            for (var color in window.leo.charts.colors) {
                OptionActions.setColor(
                    'default',
                    color,
                    window.leo.charts.colors[color]
                );
            }
        }
        ...
        var mustRebuild =
            ...
            that.metrics.length != chart.series.length;
		...
		*/
        for (let i = 0; i < that.metrics.length; i++) {
            if (
                that.metrics[i].highcharts &&
                that.metrics[i].highcharts.type == 'bar'
            ) {
                isBar = true;
            ...
            if (
                !mustRebuild &&
                chart.series[i] &&
                that.metrics[i].highcharts.type != chart.series[i].type
            ) {
                mustRebuild = true;
            }
        }
        ...
        that.metrics.forEach((metric, i) => {
            if (
                metric.highcharts &&
                metric.highcharts.yAxis &&
                !(metric.highcharts.yAxis in chart.yAxis)
            ) {
            ...
            if (metricData.rows && metricData.rows.length) {
                var yAxisIndex =
                    metric.highcharts && metric.highcharts.yAxis
                        ? metric.highcharts.yAxis
                        : 0;
                if (chart.yAxis && chart.yAxis[yAxisIndex]) {
                    chart.yAxis[yAxisIndex].setTitle({
                        text: metricData.columns[metric.id].label,
                    });
                ...
                var partitionSort =
                    metric.highcharts && metric.highcharts.sort
                        ? metric.highcharts.sort
                ...
                if (
                    ...
                    !(metric.highcharts && metric.highcharts.type === 'pie') &&
                    dateSerie.isDate(xAxisColumn)
                ) {
                    var series = dateSerie.create(
                        metric,
                        metricData,
                        chart,
                        my.dashboardOptions,
                    ...
                    if (i === 0) {
                        chart.xAxis[0].setCategories(
                            metricData.rows.map(function (e) {
                                return e[0];
                            })
                        );
                    }
                    var series = categorySerie.create(
                        metric,
                        metricData,
                        chart,
                        my.dashboardOptions,
