		...
		for(var i = 0; i < that.metrics.length; i++) {
			...
			if (metricData.rows && metricData.rows.length) {
				...
				if (chart.yAxis && chart.yAxis[yAxisIndex]) {
					chart.yAxis[yAxisIndex].setTitle({text: metricData.columns[metric.field].label});
					if (metric.field.indexOf('|percent') != -1) {
				...
				if ((chart.xAxis[0].options.type != 'category') && !(metric.highcharts && metric.highcharts.type === "pie") && dateSerie.isDate(xAxisColumn)) {
					var series = dateSerie.create(metric, metricData, chart, my.dashboardOptions, that.metrics[i].highcharts);
				} else {
					if (i === 0) {
					...
					var series = categorySerie.create(metric,metricData,chart,my.dashboardOptions,that.metrics[i].highcharts);
				}
