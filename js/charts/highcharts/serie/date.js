...
//					tickInterval: moment.duration(1, 'week').asMilliseconds(),
		...
		create: function(metric, metricData,chart,dashboardOptions,highchartSettings) {
			var column = metricData.columns[metric.field];
			var tooltip = {};
			...
			for(let j = 0; j < metricData.mapping.length; j++) {
				if(metricData.mapping[j].id == metricData.metric.field) {
					...
					for(let k = 0; k < metricData.rows.length; k++) {
						...
						if(!isNaN(timestamp)) {
							data.push([
							   moment(metricData.rows[k][0],dateType.format).valueOf(),
