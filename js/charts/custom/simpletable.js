...
class BasicRenderer extends React.Component {
    render() {
        ...
        var extraStyles =
            window.simpleTableExtraCellStyles != null
        ...
        return (
            <div
                style={{
                    ...
                    lineHeight:
                        window.simpleTableLineHeight != null
                            ...
                            : 'inherit',
                    textAlign:
                        this.props.colDef.colType == 'metric' ||
                        this.props.colDef.colType == 'fact' ||
                        this.props.colDef.colType == 'averagemoney'
                            ? 'right'
                ...
                dangerouslySetInnerHTML={{
                    __html: this.props.colDef.formatter
                        ? this.props.colDef.formatter(value)
                        : value,
                }}
            ></div>
        );
...
class CustomPinnedRowRenderer extends React.Component {
    render() {
        var val = this.props.colDef.formatter
            ? this.props.colDef.formatter(this.props.value)
            : this.props.value;
        var extraStyles =
            window.simpleTableExtraCellStyles != null
                ? window.simpleTableExtraCellStyles
...
class SimpleTable extends React.Component {
    ...
    componentDidMount() {
        ...
        if (
            this.props.spec &&
            this.props.spec.sort &&
            this.props.spec.sort[0]
        ) {
            sortBy =
                this.props.spec.sort[0].column ||
                this.props.spec.sort[0].column === 0
                    ? this.props.spec.sort[0].column
                    : sortBy;
            sortDir = this.props.spec.sort[0].direction == 'desc' ? -1 : 1;
        }
    ...
    exportData2(doPrint) {
        ...
        if (main_table.length > 1800 && doPrint) {
            alert(
                'Report is too large to print at ' +
                    main_table.length +
                    ' rows. Please filter the report to less than 1800 rows, or export the data to CSV and use Excel to print.'
            );
            return; // abort.
        }
            ...
            for (var j = 0; j < main_table[i].length; j++) {
                if (hidecols[j] == false) {
                    if (main_table[i][j] === null) {
                        ...
                        newRow.push(
                            '"' +
                                main_table[i][j].toString().replace('"', '') +
                                '"'
                        );
                        // }
                    }
        ...
        for (var j = 0; j < main_totals.length; j++) {
            if (hidecols[j] == false) {
                if (
                    main_totals[j] === null ||
                    typeof main_totals[j] === 'undefined'
                ) {
                    newRow.push('""'); // null - blank.
                } else {
                    ...
                    // else {
                    newRow.push(
                        '"' + main_totals[j].toString().replace('"', '') + '"'
                    );
                    // }
                }
            }
        ...
        if (doPrint == true) {
            downloadForm = $(
                '<div id="downloadformRSIS" style="display: none"><form method="POST" target="_blank" action="./printPage.php?name=' +
                    window.reportTitle +
                    '" /></div>'
            ).appendTo('body');
        } else {
            downloadForm = $(
                '<div id="downloadformRSIS" style="display: none"><form method="POST" action="/api/warehouse2/download" /></div>'
            ).appendTo('body');
        }
        ...
        // }
        var inputtitle = $(
            '<input type="hidden" name="title" value="' + title + '.csv"/>'
        );
        ...
        doChunk: function (sparkline) {
            ...
            for (i = 0; i < len; i++) {
                ...
                $td.highcharts('SparkLine', {
                    ...
                    tooltip: {
                        formatter:
                            typeof sparkLineToolTipFormatter == 'function'
                                ? function () {
                                      return sparkLineToolTipFormatter(
                                          this.x,
                                          this.y
                                      );
                                  }
    ...
    preProcess(props) {
        ...
        props.spec.outColumns.map((col) => {
            if (
                ...
                for (var j = 0; j < props.data.headers[0].length; j++) {
                    ...
                    col2.label = col2.label.replace(
                        '*',
                        props.data.headers[0][j].value
                    );
                    col2.value = value + ':' + j;
        ...
        outColumns.map((col, i) => {
            ...
            if (col.sparkline) {
                ...
                if (
                    col.sparkline.length == 1 &&
                    col.sparkline[0].match(/\:\*$/)
                ) {
            ...
            column.width =
                col.width || Math.floor(100 / outColumns.length) + '%'; // 60;
            ...
            if (col.export === false) {
                ...
                col.export.forEach((ex, i) => {
                    ...
                    var colName = e.value;
                    e.formatter = props.data.columns[colName]
                        ? props.data.columns[colName].formatter
                        : (n) => {
                              return n;
                          };
                    var index = 0;
                    ...
                    e.rowIndex = this.colMapping[colName]
                        ? this.colMapping[colName][index]
                        : -1;
            ...
            if (!column.func) {
                column.rowIndex = this.colMapping[column.id]
                    ? this.colMapping[column.id][column.index]
                    : -1;
            }
    ...
    handleScroll(event) {
        var scrollTop = $(event.currentTarget).scrollTop(),
            startRow =
                Math.floor(
                    scrollTop / (this.rowHeight * (this.visibleRowCount / 3))
                ) *
    ...
    adjustScrollFiller(startRow) {
        ...
        $element.find('.bottom-filler').css({
            height:
                this.rowHeight *
                (this.totalRows - startRow - this.visibleRowCount),
        });
    }
    ...
    onFilterChanged = (params) => {
        if (this.props.spec.onTotals && this.props.spec.onTotals in window) {
            ...
            outRows.forEach((row, rowNum) => {
                this.columns.forEach((column, i) => {
                    if (typeof row[i] == 'number') {
                        this.totals[i] =
                            (typeof this.totals[i] == 'undefined'
                                ? 0
                                : this.totals[i]) + parseFloat(0 + row[i]);
                    }
                });
            });
            ...
            this.columns.forEach((column, i) => {
                if (typeof column.type != 'undefined') {
                    if (column.type == 'averagemoney' && this.outRows.length) {
                        this.totals[i] =
                            Math.round(
                                (100 * this.totals[i]) / this.outRows.length
                            ) / 100;
                    }
                }
            });
            ...
            if (
                this.props.spec.onTotals &&
                this.props.spec.onTotals in window
            ) {
                totals = window[this.props.spec.onTotals](
                    totals,
                    outRows,
                    this.rows
                );
    ...
    render() {
        ...
        this.outRows = this.outRows.filter((row, i) => {
            ...
            this.columns.map((column, j) => {
                if (
                    column.filter &&
                    this.state.filters[j] !== undefined &&
                    matched
                ) {
                    ...
                    if (column.filter === true) {
                        ...
                        matched = column.filter(
                            row,
                            this.getValue.bind(this, this.rows[i]),
                            r
                        );
                    }
                }
        ...
        if (sortBy || sortBy === 0) {
            ...
            this.outRows = this.outRows.sort(
                sort.getMultiCompare(
                    [
                        {
                            direction:
                                this.state.sortDir === 1 ? 'asc' : 'desc',
                            column: sortBy,
                        },
                    ],
                    mappings
        ...
        var gridOptions = {
            ...
            headerHeight:
                window.simpleTableHeaderHeight != null
                    ...
                    : 48,
            rowHeight:
                window.simpleTableRowHeight != null
                    ? window.simpleTableRowHeight
                    : 30,
            ...
            onSelectionChanged: window.multiSelectCountChange,
        };
        ...
        var numComparator = function (num1, num2) {
            var formatNum1 = parseFloat(
                num1.toString().replace(/[^0-9.-]/g, '')
            );
            var formatNum2 = parseFloat(
                num2.toString().replace(/[^0-9.-]/g, '')
            );
        ...
        var columnDefs = this.columns.map((column, i) => {
            ...
            if (i == 0 && window.multiSelect) {
                extra.headerCheckboxSelection = true;
                extra.headerCheckboxSelectionFilteredOnly = true;
                extra.checkboxSelection = true;
            }
            return {
                minWidth: 70,
