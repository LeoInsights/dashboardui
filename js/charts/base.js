...
function getComparisonMetrics(timeframe, metric, options) {
    ...
    var timeLimit = options.notime
        ? ''
        : `and ${timeframe.dimensions.d_time}._id<=@time(${asOfDate.format(
              'HH:mm:ss'
          )}`;
    ...
    if (
        ...
        if (timeframe.value.length == 1) {
            ...
            timeframe.id = 'day';
            count = -(
                moment(timeframe.value[0]).diff(timeframe.value[1], 'days') - 1
            );
        }
        ...
        var prevStart = moment(currentStart, DATE_FORMAT)
            .subtract(count, timeframe.id)
            .format(DATE_FORMAT);
        var prevEnd = moment(currentEnd, DATE_FORMAT)
            .subtract(count, timeframe.id)
            .format(DATE_FORMAT);
        var todayLastPeriod = prevEnd;
    } else {
        var [currentStart, currentEnd] = parse_date(
            'last 0 ' + timeframe.id + ' to date',
            asOfDate
        );
        currentEnd = currentEnd || currentStart;
        var [prevStart, prevEnd] = parse_date('last ' + timeframe.id, asOfDate);
        var todayLastPeriod = parse_date(
            'today last ' + timeframe.id,
            asOfDate
        )[0];
    }
    ...
    var metrics = [
        metric +
            `|filter:${timeframe.dimensions.d_date}._id between @date(${currentStart}) and @date(${currentEnd})|${modifiers}`,
        metric +
            `|filter:${timeframe.dimensions.d_date}._id >= @date(${prevStart}) and (${timeframe.dimensions.d_date}._id < @date(${todayLastPeriod}) or (${timeframe.dimensions.d_date}._id = @date(${todayLastPeriod}) ${timeLimit}))|${modifiers}`,
        metric +
            `|filter:${timeframe.dimensions.d_date}._id between @date(${prevStart}) and @date(${prevEnd})|${modifiers}`,
    ];
...
module.exports = function (element, chart, options, my) {
    ...
    that.refreshInterval = DashboardOptions.getGroup(
        my.optiongroup
    ).refreshRate;
    ...
    var chartId =
        chart.chart_id ||
        (chart.advanced && chart.advanced.chart_id
            ? chart.advanced.chart_id
            : undefined) ||
        (chart.advanced && chart.advanced.title
            ? typeof chart.advanced.title == 'string'
                ? chart.advanced.title
                : chart.advanced.title.text
            : undefined);
    ...
    var optionsChange = function () {
        ...
        my.timeframe = $.extend(
            true,
            {},
            timeframes[optionsGroup.timebreakdown],
            {
                periods: 2,
                dimensions: {
        ...
        if (!hasColumns && my.timeframe.columns) {
            that.columns = my.timeframe.columns.map(function (col) {
                return col
                    .replace('{d_date}', my.timeframe.dimensions.d_date)
                    .replace('{d_time}', my.timeframe.dimensions.d_time);
            });
        }
        ...
        for (var j = that.filters.length - 1; j >= 0; j--) {
            ...
            if (filter.timeframe || filter.optionsGroup) {
                ...
                filter.id.match(
                    new RegExp('^' + my.timeframe.dimensions.d_date)
                )
            ) {
                hasTimeFrame = true;
            }
        ...
        if (optionsGroup.timeframe && !hasTimeFrame) {
            ...
            newFilter.id = optionsGroup.timeframe.id
                .replace('{d_date}', my.timeframe.dimensions.d_date)
                .replace('{d_time}', my.timeframe.dimensions.d_time);
            ...
            for (var i = 0; i < values.length; i++) {
                values[i] = values[i]
                    .replace('{d_date}', my.timeframe.dimensions.d_date)
                    .replace('{d_time}', my.timeframe.dimensions.d_time)
                    .replace('{periods}', my.timeframe.periods - 1);
            }
    ...
    that.showNeedsFilter = function () {
        ...
        element.append(
            $('<div/>')
                .addClass('leo-charts-loading')
                .append('<span>Select a Filter to Load Data</span>')
        );
    };
    ...
    that.start = function () {
        ...
        Data.on('loading', function (id) {
            if (id === my.graphWatching.id) {
                ...
                that.showLoading("<i class='fa fa-spinner fa-spin fa-2x'></i>");
            }
        });
    ...
    my.getMetric = that.getMetric = function (metricNumber) {
        ...
        for (var i in my.dataSources) {
            ...
            if (!dataSource.error) {
                ...
                for (
                    let j = 0;
                    j < (dataSource.columnheaders || []).length;
                    j++
                ) {
    ...
    my.changeChart = that.changeChart = function (
        newParams,
        replace,
        keepFilters
    ) {
